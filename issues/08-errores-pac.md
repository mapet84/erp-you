...
## Acceptance criteria
- [ ] Captura de errores de `facturama-client.createCfdi` y mapeo de los códigos/causas comunes (RFC/Nombre/CP/Régimen no coinciden, RFC no válido, etc.) a mensajes en español.
- [ ] `Invoice` queda en estatus `error` con el detalle del PAC (`errorPac`) cuando el timbrado falla.
- [ ] La UI muestra el mensaje accionable y permite corregir los datos y reintentar conservando lo capturado.
- [ ] Un error del PAC no consume el folio de forma definitiva: el reintento exitoso del mismo folio sí timbra.
