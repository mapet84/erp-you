...
import { crearUnidad, crearConversion, borrarConversion } from "../actions";
      ...
      <section className="space-y-3">
        ...
        <div className="overflow-hidden rounded-xl border border-neutral-200 bg-white">
          <table className="w-full text-sm">
            <thead className="bg-neutral-50 text-left text-neutral-500"><tr><th className="px-4 py-2 font-medium">Código</th><th className="px-4 py-2 font-medium">Nombre</th></tr></thead>
            <tbody>
              {unidades.map((u) => (
                <tr key={u.id} className="border-t border-neutral-100"><td className="px-4 py-2 font-mono text-neutral-800">{u.codigo}</td><td className="px-4 py-2 text-neutral-800">{u.nombre}</td></tr>
              ))}
            </tbody>
          </table>
        </div>
